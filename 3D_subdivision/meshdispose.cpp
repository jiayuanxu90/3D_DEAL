...
void MeshDispose::edge_subdivide_butterfly(Edge *&current, Model *&model)
{
    if (current->flag == UNSUBDIVIDED) {
        ...
        if (edge_Q1 == NULL) {
            ...
            unsigned int i_q1 = edge_Q1->next->data.y();
        ...
        if (edge_Q2 == NULL) {
            ...
            unsigned int i_q2 = edge_Q2->next->data.y();
        ...
        if (edge_rev != NULL) {
            ...
            if (edge_rev->prev->reverse != NULL) {
                unsigned int i_q3 = edge_rev->prev->reverse->next->data.y();
            ...
            if (edge_rev->next->reverse != NULL) {
                unsigned int i_q4 = edge_rev->next->reverse->next->data.y();
...
void MeshDispose::face_subdivide_to_four(Face * &current, HalfEdge * &halfEdge, Model * &model)
{
    ...
    Edge * edge_new_1 = new Edge(QVector2D(face_new_1->e2->data.y(), face_new_1->e1->data.x()));
    ...
    model->add_v_index(face_new_1->e1->data.x());
    model->add_v_index(face_new_1->e2->data.x());
    model->add_v_index(face_new_1->e3->data.x());

    Face * face_new_2 = new Face();
    face_new_2->e1 = current->e2->child2;
    face_new_2->e2 = current->e3->child1;
    Edge * edge_new_2 = new Edge(QVector2D(face_new_2->e2->data.y(), face_new_2->e1->data.x()));
    ...
    model->add_v_index(face_new_2->e1->data.x());
    model->add_v_index(face_new_2->e2->data.x());
    model->add_v_index(face_new_2->e3->data.x());

    Face * face_new_3 = new Face();
    face_new_3->e1 = current->e3->child2;
    face_new_3->e2 = current->e1->child1;
    Edge * edge_new_3 = new Edge(QVector2D(face_new_3->e2->data.y(), face_new_3->e1->data.x()));
    ...
    model->add_v_index(face_new_3->e1->data.x());
    model->add_v_index(face_new_3->e2->data.x());
    model->add_v_index(face_new_3->e3->data.x());

    Edge * edge_rev_1 = new Edge(QVector2D(face_new_1->e3->data.y(), face_new_1->e3->data.x()));
    Edge * edge_rev_2 = new Edge(QVector2D(face_new_2->e3->data.y(), face_new_2->e3->data.x()));
    Edge * edge_rev_3 = new Edge(QVector2D(face_new_3->e3->data.y(), face_new_3->e3->data.x()));
    ...
    model->add_v_index(face_new_4->e1->data.x());
    model->add_v_index(face_new_4->e2->data.x());
    model->add_v_index(face_new_4->e3->data.x());
}
