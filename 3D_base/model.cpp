    ...
    if (this!=NULL && !empty()) {
        ...
        for (int i=0; i<node_size(); ++i) {
            if (!normal_indices.empty() && normal_indices.size()!=0) {
                glNormal3f(vertices_normals.at(normal_indices.at(i)).x()/range,
                           vertices_normals.at(normal_indices.at(i)).y()/range,
                           vertices_normals.at(normal_indices.at(i)).z()/range);
            }
            glVertex3f(vertices.at(vertex_indices.at(i)).x()/range,
                       vertices.at(vertex_indices.at(i)).y()/range,
                       vertices.at(vertex_indices.at(i)).z()/range);
        }
        glEnd();
    }
